...
function Panel({ className = "", superTitle, title, description, orientation, imageSrc }: {
  className: string,
  superTitle: string,
  title: string,
  description: any,
  ...
  const image = <div className="lg:pr-4">
    <div className="relative overflow-hidden rounded-3xl bg-gray-900 px-6 pb-9 pt-64 shadow-2xl sm:px-12 lg:max-w-lg lg:px-8 lg:pb-8 xl:px-10 xl:pb-10">
      <Image alt="team image" src={imageSrc} className="absolute inset-0 h-full w-full object-cover brightness-125 saturate-0" />
    </div>
  </div>
  return <div className={`mx-auto grid max-w-2xl grid-cols-1 items-start gap-x-8 gap-y-16 sm:gap-y-24 lg:mx-0 lg:max-w-none lg:grid-cols-2 ${className}`}>
    {orientation == 'left' ? <>{body}{image}</> : <>{image}{body}</>}
  </div>
}
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className='flex flex-col justify-center items-center'>
          ...
          <div className="mx-auto max-w-2xl text-center lg:mx-0 ">
            <h2 className="text-3xl  font-bold tracking-tight text-slate-100 sm:text-4xl">{hero.title}</h2>
